...
class CADSynth(Dataset):
    ...
    def _get_filenames(self, root_dir, filelist):
        ...
        # # 新增：如果是训练集且 sample_ratio < 1.0，则随机抽取子集
        # if self.split == "train" and self.sample_ratio < 1.0:
        #     random.seed(42)  # 固定种子，保证可复现
        #     random.shuffle(self.file_paths)
        #     n = int(len(self.file_paths) * self.sample_ratio)
        #     self.file_paths = self.file_paths[:n]
        #     print(f"Using {n} files for training (sample_ratio={self.sample_ratio})")

        # 对所有 split 进行采样（包括 val 和 test）
        if self.sample_ratio < 1.0:
            random.seed(42)  # 固定种子，保证可复现
            random.shuffle(self.file_paths)
            n = int(len(self.file_paths) * self.sample_ratio)
            self.file_paths = self.file_paths[:n]
            print(f"Using {n} files for {self.split} (sample_ratio={self.sample_ratio})")
    ...
    def load_one_graph(self, file_path):
        ...
        # 生成伪实例ID
        face_labels_np = graph.ndata["f"].cpu().numpy()  # [num_nodes]
        dense_adj_np = dense_adj.cpu().numpy()  # [num_nodes, num_nodes]
        pseudo_instance_id = self.generate_pseudo_instance(dense_adj_np, face_labels_np)
        pyg_graph.instance_id = torch.from_numpy(pseudo_instance_id).type(torch.int)  # 保存实例ID
    ...
    def generate_pseudo_instance(self, adj_matrix, face_labels):
        """
        adj_matrix: numpy array [N, N] 邻接矩阵 (0/1)
        face_labels: numpy array [N] 面类别标签
        返回: numpy array [N] 伪实例ID
        """
        N = len(face_labels)
        instance_id = np.full(N, -1, dtype=int)
        current_id = 0
        ...
        for label in unique_labels:
            ...
            n_components, labels_sub = connected_components(
                csgraph=csr_matrix(sub_adj), directed=False, return_labels=True
            )
            for comp in range(n_components):
                comp_nodes = indices[labels_sub == comp]
                instance_id[comp_nodes] = current_id
                current_id += 1
        return instance_id
