...
def collator(items, multi_hop_max_dist, spatial_pos_max):  #items({PYGGraph_1, PYGGraph_1_mian}, {PYGGraph_2, PYGGraph_2_mian}, ..., PYGGraph_batchsize)
    ...
    # 将 instance_ids 填充到统一长度（用 -1 填充无效位置）
    instance_id_list = []
    for inst_id in instance_ids:
        N = inst_id.size(0)
        padded = torch.full((max_node_num,), -1, dtype=torch.long)  # 填充 -1
        padded[:N] = inst_id
        instance_id_list.append(padded)
    instance_ids_padded = torch.stack(instance_id_list, dim=0)  # [batch_size, max_node_num]

    # 构建实例邻接矩阵 inst_adj
    inst_adj_list = []
    for inst_id in instance_ids:
        N = inst_id.size(0)
        # 初始化一个全零矩阵，大小 [max_node_num, max_node_num]
        adj = torch.zeros(max_node_num, max_node_num, dtype=torch.long)
        # 填充真实节点部分（N 是当前样本的实际节点数）
        for i in range(N):
            for j in range(N):
                if i != j and inst_id[i] == inst_id[j]:
                    adj[i, j] = 1
        inst_adj_list.append(adj)
    inst_adj = torch.stack(inst_adj_list, dim=0)  # [batch_size, max_node_num, max_node_num]
